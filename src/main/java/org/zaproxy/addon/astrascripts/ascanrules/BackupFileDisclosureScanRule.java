...
public class BackupFileDisclosureScanRule extends AbstractAppPlugin {
    ...
    public void scan() {
        ...
        if (astraAddOnConstants.is404status(this.getBaseMsg())) {
    ...
    private boolean checkNotFoundError(HttpMessage originalMessage) {
        ...
        try {
            ...
            String toFindFileName = "lmaotestPageToCheckFora404WhichShouldNotExist";
            String toFindFilePath = temppath.substring(0, slashposition) + "/" + toFindFileName;

            LOG.debug("Trying to get 404 error through file: {}", toFindFilePath);
            HttpMessage doesNotExist =
                    new HttpMessage(
                            new URI(
                                    ...
                                    null,
                                    null));
            setMessageCookies(doesNotExist, originalMessage);
            sendAndReceive(doesNotExist, false);
            if (doesNotExist.getResponseHeader().getStatusCode() != HttpStatusCode.NOT_FOUND) {
                return true;
            }
            return false;
        } catch (Exception e) {
